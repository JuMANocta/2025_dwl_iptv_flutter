...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.16.2+110-blue?style=flat-square"/>
...
- [ ] **Lecture 4K saccadée sur box TV** (§video4k) — **diagnostiqué le 2026-08-30 sur la box** : le décodage matériel fonctionne, mais l'image passe par une **copie mémoire** à chaque trame (`mediacodec-copy`) et une image sur trois est jetée à l'affichage. Le taux de perte est **identique quel que soit le débit du flux**, ce qui écarte le réseau et la source : c'est la sortie vidéo qui ne suit pas. Un **banc d'essai** est disponible dans Paramètres → Optimisation pour comparer les chemins de rendu sur son propre matériel (⚠️ le mode « Direct » supprime les sous-titres). Quatre chemins de rendu ont été comparés sur la box : le décodage matériel n'est pas en cause et la synchro reste parfaite, ce sont des images **déjà prêtes** qui sont jetées à l'affichage. Le mode « Direct » s'est révélé **instable sur cette box** (l'application s'arrête au bout de quelques secondes) et un garde-fou remet désormais le rendu par défaut tout seul si l'application s'arrête pendant une lecture de diagnostic. Piste en cours : le travail HDR imposé au processeur graphique, réglable dans le même banc sans rien perdre. Le plantage signalé à l'origine n'a pas encore été reproduit
