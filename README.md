...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.18.11+144-blue?style=flat-square"/>
...
### ✅ Terminé
- [x] **Sortir d'un film sans que l'accueil gèle** (§homeIndex + §inferDelta, 2026-09-06) — Sur le téléviseur, revenir d'une série laissait l'écran figé plus d'une seconde : ce n'était pas le lecteur, c'était l'accueil derrière lui qui se reconstruisait entièrement. Deux causes mesurées : la reprise sauvegardée faisait recalculer le bandeau du haut sur toutes les pages, en ré-indexant tous les titres à chaque fois ; et une catégorie apprise grâce à TMDB (le film qu'on venait d'ouvrir, s'il vient d'une liste qui ne range pas ses titres) refaisait le rangement complet des trois onglets. L'index est désormais construit une fois par liste, et une catégorie apprise ne déplace que le titre concerné. Mesuré : le retour passe de ~1,4 s de calcul à moins de 0,1 s sur l'émulateur ; le changement d'onglet n'a plus de ré-indexation
